...
pub(crate) struct UrlEncodedQuery<'a> {
...
impl UrlEncodedQuery<'_> {
    ...
    pub(crate) fn with(value: &impl Serialize) -> Result<Self, serde_urlencoded::ser::Error> {
    ...
    pub(crate) fn set_url(mut self, url: &mut Url) {
    ...
    pub(crate) fn append_pair(&mut self, name: &str, value: &str) -> &mut Self {
        self.urlencoded.append_pair(name, value);
        self
    }

    /// Serialize and append the name of a parameter without any value.
    pub(crate) fn append_key_only(&mut self, name: &str) -> &mut Self {
    ...
    pub(crate) fn extend_keys_only<I, K>(&mut self, iter: I) -> &mut Self
    where
        I: IntoIterator,
        I::Item: Borrow<K>,
        K: AsRef<str>,
